...
###[Singleton](https://github.com/DemidenkoAndrey/Design-Pattern-in-Ruby/tree/master/singleton)

######creational design pattern

The Singleton pattern can help us deal with the cases where there is only one of something.
There are two characteristics that make a singleton a singleton: A singleton class
...
methods. For example, we could get some of the singleton behavior from global variables
or constants, although these elements lack the uniqueness characteristic that
makes a real singleton a singleton. In addition, we can build singletons from class- or
module-level methods and variables.

Ruby includes a [singleton](http://ruby-doc.org/stdlib/libdoc/singleton/rdoc/Singleton.html).
